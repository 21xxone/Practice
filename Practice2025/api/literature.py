...
from schemas import SourceBase, SourceCreate, SourceTypeBase, SourceRead
...
def create_source(source: SourceCreate, db: Session = Depends(get_db)):
    # собираем только непустые поля, остальные будут NULL в БД
    payload = source.dict(exclude_none=True)
    db_source = Source(**payload)
    db.add(db_source)
    db.commit()
